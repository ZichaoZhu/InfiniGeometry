...
## 2026-09-23 exp6 交接总览与方法边界补充

### 实验简述

为方便接收方理解实验路线，在交接索引补充 Exp6-1～Exp6-5 的目的、实际做法和结论边界，并汇总 Exp6-4 与官方 MoGe3 SSR 的差别。只整理既有证据，没有新增训练、评估或数值结论。

### 实验结果

- [实验总览与 SSR 对照](../docs/experiment_handoff_20260923.md)
- [Exp6-4 完整方法边界与源码入口](../docs/exp6_4_disparity_adapter_handoff.md)

### 其他

Exp6-4 只复用官方稀疏网络主体，保留 disparity 表示、限幅、损失及迭代间梯度，Base 全程冻结；Exp6-5 R1 的几何损失修复未改变 Exp6-4。区分官方发布权重与 Exp6-5 自行训练的官方实现组，不将人工难例等同于完整 benchmark 或严格 OOD。此前代码已获授权推送，本次说明增补尚未提交、推送；历史条目和原有无关改动均保留。

## 2026-09-23 exp6-4 稀疏数值非确定性限时诊断

### 实验简述

按一小时内的授权，在 S115 GPU1 独立目录用同一真实权重和一张缓存 Val 图复测。默认同进程及跨进程均有差异，首次出现在稀疏池化；固定输入重放确认邻居集合相同、求和顺序变化。临时固定池化输出坐标并开启 PyTorch 确定性后，本例预测、梯度与一步更新跨进程精确一致。

### 实验结果

- [诊断报告、对照统计与复现脚本](./exp6_4_official_ssr_disparity_adapter/diagnosis_20260923_determinism/README.md)

### 其他

复用既有验收工具和选核缓存，只增加独立诊断钩子；8 个进程实际选核一致，未改正式源码、配置、权重或安装包。3 项 CPU 统计自测通过，GPU 已释放，不干预他人任务。默认严格一致性仍未通过；组合控制仅在单图、新建 optimizer 的一步检查中通过，不代表全量精度、长期恢复或其他硬件已验收。原失败证据及诊断脚本 v1 的形状参数错误均保留，未提交或推送。

## 2026-09-23 exp6 交付总结与提交

### 实验简述

面向师兄扩训交接，汇总 Exp6-1～Exp6-5 的路线，推荐 Exp6-4 official_flex 作为当前主线，并明确它是官方 SSR 网络主体的 disparity 适配版。独立诊断代码已提交为 `8e2dac9`，报告、证据与交付说明单独作结果提交；正式方法不变。

### 实验结果

- [交接索引、实验总览与外部资产清单](../docs/experiment_handoff_20260923.md)
- [Exp6-4 方法边界](../docs/exp6_4_disparity_adapter_handoff.md)
- [数值诊断报告](./exp6_4_official_ssr_disparity_adapter/diagnosis_20260923_determinism/README.md)

### 其他

本轮重新执行 7 项路径准备测试和 2 项源码核验工具测试，均通过；40 个证据哈希、5 个脚本 AST、41 个 JSON 及文档链接检查通过，没有重跑 GPU 探针。默认稀疏实现的严格一致性仍未通过；临时组合控制仅在单图、单步上验证，尚未进入正式实现。当前 refiner-only 为单卡，多卡扩训与接收方环境验收另行开展。两次提交使用用户 Git 身份；本轮仅本地提交，未推送、部署或操作服务器，用户原有日志改动不纳入提交。
