...
2026-09-23 收尾：official_flex 单组入口、显式机器路径与运行说明已整理。S115 现有环境中的 CUDA 功能、冻结和第 3→6 步恢复验证通过；历史权重的严格数值对照未通过，同版本复测也有差异，详见[验证报告](./verification_20260923/README.md)。本次按现状交接，不继续诊断；尚未验收全新机器安装环境或多卡扩训。
...
- S115 现有环境已完成 51 项相关 CPU、10 项 CUDA 测试及独立短步恢复。模型、optimizer、RNG 和采样恢复通过；本机不重复执行 CUDA，旧部署报告不计作本次测试。
- 同一历史 checkpoint 的 K0 精确一致，K1/3/5 未通过严格数值对照。原始失败证据随交接保留，不将功能通过写成全部通过。
- 权重、Hypersim 数据和固定评估资源不随 Git 分发，接收方需确认访问权限，并在目标环境先执行路径检查和短测。
- 当前仅支持单卡 refiner-only。GPU 扩展、全新环境安装验收、去限幅和新训练目标属于独立工作，本次不启动。
