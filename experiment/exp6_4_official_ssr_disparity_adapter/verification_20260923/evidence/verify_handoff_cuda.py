...
def compare_tree(left, right, *, tolerance, prefix="root"):
    ...
    if isinstance(left, torch.Tensor):
        ...
        if left.is_floating_point():
            torch.testing.assert_close(left, right, atol=tolerance, rtol=tolerance, msg=prefix)
...
def probe(args):
    ...
    maximum_residual = max(float(value.abs().max()) for value in output.bounded_residuals)
...
def compare_resumes(args):
    ...
    assert generator.sample_indices(8, 8) == b["stage"]["expected_next_global_indices"]
...
def main():
    ...
    parser.add_argument("mode", choices=("probe", "compare-probes", "compare-resumes"))
    ...
    result = {"probe": probe, "compare-probes": compare_probes, "compare-resumes": compare_resumes}[args.mode](args)
