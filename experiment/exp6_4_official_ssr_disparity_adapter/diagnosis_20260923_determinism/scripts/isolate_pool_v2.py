...
def main():
    ...
    for mode in ('default', 'canonical'):
        ...
        for _ in range(3):
            with torch.no_grad():
                output_shape = torch.Size([shape[0], *((int(x) + 1) // 2 for x in shape[1:4]), shape[-1]])
                out, out_coords, _, cache = pool(feats, coords, shape, output_coords=expected if mode == 'canonical' else None, output_shape=output_shape)
